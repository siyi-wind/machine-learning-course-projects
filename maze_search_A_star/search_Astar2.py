...
def CalcF(point, minp, endp):
    h = math.sqrt((endp.x-point.x)**2+(endp.y-point.y)**2)
    g = 0
...
def notObstacleAndClose(point,closePointList):
    if point.status != ITEM_STAT_OBSTACLE and point not in closePointList:
        return True
    return False
...
def findSurroundPoint(point,closePointList):
    ...
    if point.x < gameMapWidth-1 and point.y < gameMapHeight-1:
        ...
        if notObstacleAndClose(rightDown,closePointList):
